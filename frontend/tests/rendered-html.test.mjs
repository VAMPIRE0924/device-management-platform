...
test("keeps formal login, MFA and editable deployment settings", async () => {
  ...
  contains(page, ["修改初始密码", "绑定并验证邮箱", "开启双重认证", "认证器令牌", "邮箱验证码", "保存系统设置", "SMTP 主机", "SMTP 密码", "证书文件路径", "面板地址", "反代地址"]);
...
test("shows all node tunnels with independent state and activity", async () => {
  const page = await readFile(pageURL, "utf8");
  contains(page, ["节点实际返回的 SOCKS 通道", "未绑定", "运行中", "已关闭", "活跃中", "非活跃", 'className="socks-table"', "api.setManagedTunnel", "markProjectTunnelOpen(project)"]);
