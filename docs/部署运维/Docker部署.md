...
这是 Chrome Safe Browsing 的站点声誉或内容判定页面，不是 TLS 证书错误。v1.0.11 使用 32 个长期稳定的 `web-01` 至 `web-32` 访问入口，不再按用户或 Endpoint 生成新的哈希登录子域；入口复用时仍通过一次性授权、来源绑定、有效平台登录和独立设备 Cookie 命名空间隔离会话。设备域授权完成页及上游 HTML 页面会明确显示 I5CLOUD 代理关系。已被 Google 标记的旧 `device-*` 主机不会因升级立即清除历史标记；部署新版后还应由 `66yoyo.com` 的 Search Console 所有者在“安全性问题”中提交复核。
