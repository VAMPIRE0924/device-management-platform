...
本文适用于 `vampirerune/device-management-platform:v1.0.3`。生产形态为单容器、单实例 SQLite；不支持多个应用实例同时写同一数据卷。
...
```dotenv
DMP_IMAGE=vampirerune/device-management-platform:v1.0.3
...
docker run --rm \
  ...
  vampirerune/device-management-platform:v1.0.3 \
